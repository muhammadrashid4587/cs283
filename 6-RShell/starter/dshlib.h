...
#define EXE_MAX 64
#define ARG_MAX 256
#define CMD_MAX 8         // max # of piped commands
#define CMD_ARGV_MAX 32   // max tokens per single command

/* Shell prompt (can change to match assignment specs) */
#define SH_PROMPT "dsh4> "

/* Built-in commands */
#define EXIT_CMD "exit"

/* Return codes for internal use */
#define OK 0
#define WARN_NO_CMDS -1
#define ERR_TOO_MANY_COMMANDS -2
#define ERR_MEMORY -3
...
typedef struct cmd_buff {
    ...
    char *_cmd_buffer;            // allocated copy of the entire command line
} cmd_buff_t;

typedef struct command {
    cmd_buff_t buff;              // parsed arguments
    char *infile;                 // if you implement redirection
    char *outfile;                // if you implement redirection
    int  append;                  // for >> 
} command_t;
...
/* Prototypes for local shell logic */
