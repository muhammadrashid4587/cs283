...
#include "rshlib.h"   // RDSH_EOF_CHAR, RDSH_COMM_BUFF_SZ, ...
#include "dshlib.h"   // for SH_PROMPT (or anything else you want to reuse)

/*
 * exec_remote_cmd_loop:
 *   - Connect to server (server_ip, port).
 *   - Repeatedly prompt user, read command, send with '\0'.
 *   - Receive server's output until RDSH_EOF_CHAR, print it.
...
int exec_remote_cmd_loop(const char *server_ip, int port)
{
    ...
    while (1) {
        ...
        // send the command + null terminator
        size_t send_len = strlen(cmdline) + 1; // +1 for '\0'
        if (send(sockfd, cmdline, send_len, 0) < 0) {
            perror("send");
            break;
        }

        // if user typed "exit", we can break
        if (strcmp(cmdline, "exit") == 0) {
            // we won't read anything else from server 
            // (the server might just close or read further).
            break;
        }

        // read server output until we see RDSH_EOF_CHAR
        char recv_buf[RDSH_COMM_BUFF_SZ];
        while (1) {
            ssize_t rbytes = recv(sockfd, recv_buf, sizeof(recv_buf), 0);
            if (rbytes < 0) {
                perror("recv");
                goto done;
            }
            ...
            // check if last character is EOF_CHAR
            int got_eof = 0;
            if ((unsigned char)recv_buf[rbytes - 1] == (unsigned char)RDSH_EOF_CHAR) {
                // replace with '\0'
                recv_buf[rbytes - 1] = '\0';
                got_eof = 1;
            }
            // print
            printf("%.*s", (int)rbytes, recv_buf);
            fflush(stdout);

            if (got_eof) {
                // done reading this command's output
                break;
            }
        }
    }
