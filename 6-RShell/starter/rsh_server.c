...
#include "rshlib.h"    // RDSH_EOF_CHAR, RDSH_DEF_PORT, ...
#include "dshlib.h"    // build_cmd_list(), etc.

static int exec_cli_requests(int client_sock);

/* Return codes for exec_cli_requests */
#define SVR_OK        0
#define SVR_STOP      1

/*
 * start_server:
 *   - Create a socket, bind to server_interfaces:port, listen.
 *   - Accept a client, call exec_cli_requests(client_sock).
 *   - If it returns SVR_STOP, break out & close server.
 */
int start_server(const char *server_interfaces, int port)
{
    int server_sock;
    ...
    if (inet_pton(AF_INET, server_interfaces, &srv_addr.sin_addr) <= 0) {
    ...
    while (1) {
        ...
        // handle one client
        int rc = exec_cli_requests(client_sock);
        close(client_sock);

        if (rc == SVR_STOP) {
            // "stop-server" command => shut down
            printf("Shutting down server...\n");
            break;
        }
        // else keep accepting next client
    }
...
/*
 * exec_cli_requests:
 *   - In a loop, read bytes from client until '\0'.
 *   - That is one command. If "exit", end. If "stop-server", return SVR_STOP.
 *   - Else parse & fork/exec (like local shell), but dup output to client_sock.
 *   - Then send 1 byte RDSH_EOF_CHAR to mark end of output.
 */
static int exec_cli_requests(int client_sock)
{
    ...
    while (1) {
        ssize_t rbytes = recv(client_sock, recv_buf, sizeof(recv_buf), 0);
        if (rbytes < 0) {
            perror("recv");
            return SVR_OK;
        }
        ...
        // accumulate into cmdline until we see '\0'
        for (int i = 0; i < rbytes; i++) {
            if (recv_buf[i] == '\0') {
                // end of command
                cmdline[cmd_pos] = '\0';

                // handle built-ins: exit or stop-server
                if (strcmp(cmdline, "exit") == 0) {
                    printf("Client requested exit.\n");
                    return SVR_OK;
                }
                if (strcmp(cmdline, "stop-server") == 0) {
                    printf("Client requested stop-server.\n");
                    return SVR_STOP;
                }

                // parse into pipeline
                command_list_t clist;
                build_cmd_list(cmdline, &clist);

                if (clist.num == 0) {
                    // no commands => just send EOF
                    send(client_sock, &RDSH_EOF_CHAR, 1, 0);
                }
                else if (clist.num == 1) {
                    ...
                    if (cmd->argc > 0 && strcmp(cmd->argv[0], "cd") == 0) {
                        if (cmd->argc == 2) {
                            if (chdir(cmd->argv[1]) != 0) {
                                dprintf(client_sock, "cd error: %s\n", strerror(errno));
                            }
                        } else {
                            dprintf(client_sock, "usage: cd <directory>\n");
                        }
                    ...
                    for (int c = 0; c < clist.num; c++) {
                        cmd_buff_t *cmd = &clist.commands[c].buff;
                        if (c < clist.num - 1) {
                            if (pipe(pipe_fd) < 0) {
                                perror("pipe");
                                // handle error
                        ...
                        if (pid == 0) {
                            // child
                            // if not first, read from in_fd
                            if (c > 0) {
                                dup2(in_fd, STDIN_FILENO);
                                close(in_fd);
                            }
                            // if not last, pipe out
                            if (c < clist.num - 1) {
                                dup2(pipe_fd[1], STDOUT_FILENO);
                                close(pipe_fd[0]);
                                close(pipe_fd[1]);
                            } else {
                                // last command => output to client sock
                                dup2(client_sock, STDOUT_FILENO);
                                dup2(client_sock, STDERR_FILENO);
                            }
                            execvp(cmd->argv[0], cmd->argv);
                    ...
                    for (int c = 0; c < clist.num; c++) {
                        ...
                        free(clist.commands[c].buff._cmd_buffer);
                    }
                    // send EOF
                    send(client_sock, &RDSH_EOF_CHAR, 1, 0);
                }
                ...
                // keep accumulating
                if (cmd_pos < (int)sizeof(cmdline) - 1) {
                    cmdline[cmd_pos++] = recv_buf[i];
                }
                // else discard (command too long)
            }
        }
    }
    // never gets here in normal usage
    return SVR_OK;
}
