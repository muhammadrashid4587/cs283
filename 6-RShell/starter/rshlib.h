...
/* Default port/IPs (may be overridden by -p / -i command-line flags) */
#define RDSH_DEF_PORT           1234
#define RDSH_DEF_SVR_INTFACE    "0.0.0.0"   // listen on all interfaces
#define RDSH_DEF_CLI_CONNECT    "127.0.0.1" // client connects to localhost by default

/* Buffer sizes for sending/receiving over the network */
#define RDSH_COMM_BUFF_SZ 512

/*
 * RDSH_EOF_CHAR: ASCII 0x04 (End-of-Transmission / EOT).
 * The server sends this after finishing command output.
 */
static const char RDSH_EOF_CHAR = 0x04;

/*
 * Prototypes for remote code:
 *  - Client
 *  - Server
 */
